...
import {
  ...
  TemplateRowActions,
  TemplateSourceCell,
  TemplateValidationStatusCell,
  TemplateVersionHistoryContent,
  TemplateVersionDetail,
  TemplateVersionVariablesCell,
  OverviewPage,
  MatchGroupsPage,
  MatchGroupReferenceList,
  MatchGroupReferenceStatusCell,
  OrganizationPage,
  ProvidersPage,
  QueueMonitorPage,
  RouteStrategyPage,
  ...
  templateRouteDependencyFromRules,
  ...
  matchGroupReferencesFromRouteRules,
  ...
  performanceConcurrencyConfirmation,
  performanceDiagnosticGaugeRows,
  performanceStageRowsForSelection,
  settingEditorConfigForKey,
  settingInputValueFromEditor,
  summarizeSourceDedupe,
  withSortableColumns,
} from "./ConsolePages";
import type {
  ChannelApiRecord,
...
describe("critical console pages", () => {
  ...
  it("renders overview and queue monitoring shells with localized metric copy", () => {
    ...
    const monitoringMarkup = renderPage(
      <ConsolePages.MonitoringPage
        lastUpdated={lastUpdated}
        onRefresh={() => undefined}
        activeSubTab="dead-letters"
      />,
    );
    expect(monitoringMarkup).toContain("死信列表");
    expect(monitoringMarkup).toContain("处理模式");
  ...
  it("does not expose a fake status filter on audit logs", () => {
    const auditMarkup = renderPage(
      <ConsolePages.AuditPage
        lastUpdated={lastUpdated}
        onRefresh={() => undefined}
      />,
    );

    expect(auditMarkup).toContain("操作审计");
    expect(auditMarkup).not.toContain("全部状态");
    expect(auditMarkup).not.toContain("排队中");
    expect(auditMarkup).not.toContain("死信");
  });

  it("renders message logs with one lifecycle status column", () => {
    const markup = renderPage(
      <ConsolePages.MessageLogsPage
        lastUpdated={lastUpdated}
        onRefresh={() => undefined}
      />,
    );
    const statusIndex = markup.indexOf("状态");
    const errorIndex = markup.indexOf("错误码");

    expect(markup).toContain("消息日志");
    expect(markup).toContain("状态");
    expect(markup).toContain("出站时间");
    expect(statusIndex).toBeGreaterThan(-1);
    expect(errorIndex).toBeGreaterThan(statusIndex);
    expect(markup).not.toContain("入站状态");
    expect(markup).not.toContain("出站状态");
  });

  it("renders collapsible message detail sections with inbound payload collapsed by default", () => {
    const markup = renderPage(
      <MessageLogDetailContent
        selected={{
          id: "message-1",
          traceId: "trace-1",
          source: "来源 A",
          receivedAt: "2026-06-04 10:00:00",
          status: "queued",
          inboundStatus: "planned",
          matchedRoute: "生产路由",
          outboundStatus: "queued",
          firstOutboundAt: "2026-06-04 10:00:02",
          lastOutboundAt: "2026-06-04 10:00:02",
          targetProvider: "Webhook",
          duration: "2000 ms",
        }}
        selectedDetail={{
          id: "message-1",
          ...
          source_id: "source-1",
          source_name: "来源 A",
          received_at: "2026-06-04T02:00:00Z",
          status: "queued",
          inbound_status: "planned",
          matched_flow_name: "生产路由",
          outbound_status: "queued",
          first_outbound_at: "2026-06-04T02:00:02Z",
          last_outbound_at: "2026-06-04T02:00:02Z",
          payload: { title: "告警", nested: { level: "high" } },
          payload_hash: "hash",
          attempts: [],
          timeline: [
            {
    ...
    expect(markup).not.toContain("最近出站时间");
  });
  ...
  it("renders source and provider pages with chinese status and platform mappings", () => {
    ...
    expect(sourcesMarkup).toContain(
      "创建下级独立推送接口，管理下级推送行为，查看最近入站 Payload。",
    );
    expect(sourcesMarkup).not.toContain("管理下级系统来源、鉴权");
    expect(sourcesMarkup).toContain("鉴权方式");
    expect(sourcesMarkup).toContain("来源列表");
    expect(sourcesMarkup).not.toContain(
      "操作列可查看最近Payload，也可发起入站测试。",
    );
    expect(sourcesMarkup).not.toContain("token_and_hmac");
    expect(providersMarkup).toContain("推送渠道");
    expect(providersMarkup).toContain(
      "现已支持企业微信、飞书、钉钉、邮箱、短信、主流开源推送渠道和平台级联。",
    );
    expect(providersMarkup).not.toContain(
      "配置企业微信、飞书、钉钉、邮箱、短信、Webhook、自建服务和 MVP-PUSH 推送渠道。",
    );
    ...
    expect(providersMarkup).not.toContain("全部推送渠道类型");
    const sourceListMarkup = sourcesMarkup.slice(
      sourcesMarkup.indexOf("来源列表"),
    );
    const dedupeIndex = sourceListMarkup.indexOf("入站去重");
    const rateLimitIndex = sourceListMarkup.indexOf("入站限流");
    const allowlistIndex = sourceListMarkup.indexOf("IP 白名单");
    expect(dedupeIndex).toBeGreaterThan(-1);
    expect(rateLimitIndex).toBeGreaterThan(dedupeIndex);
    expect(allowlistIndex).toBeGreaterThan(rateLimitIndex);
    const providerListMarkup = providersMarkup.slice(
      providersMarkup.indexOf("推送渠道实例列表"),
    );
    const providerNameIndex = providerListMarkup.indexOf("推送渠道名称");
    const providerTypeIndex = providerListMarkup.indexOf("推送渠道类型");
    const retryIndex = providerListMarkup.indexOf("允许重试次数");
    const concurrencyIndex = providerListMarkup.indexOf("并发");
    const enabledIndex = providerListMarkup.indexOf("状态", concurrencyIndex);
    ...
    expect(concurrencyIndex).toBeGreaterThan(retryIndex);
    expect(providerListMarkup).not.toContain("死信策略");
    expect(enabledIndex).toBeGreaterThan(concurrencyIndex);
    expect(actionIndex).toBeGreaterThan(enabledIndex);
  });

  it("renders provider token cache health beside the channel name instead of the enabled column", () => {
    const record = {
      ...createProviderDraft("wecom_app", 1),
      name: "企微应用告警",
      is_cached: false,
      token_cache_status: "expired",
    };
    const nameMarkup = renderPage(<ProviderNameCell record={record} />);
    ...
    expect(nameMarkup).toContain("企微应用告警");
    expect(nameMarkup).toContain("provider-token-dot");
    expect(nameMarkup).toContain("provider-token-dot--warning");
    expect(nameMarkup).toContain('title="AccessToken：已过期"');
    expect(nameMarkup).not.toContain("ant-tag");
    expect(enabledMarkup).toContain("启用");
    expect(enabledMarkup).not.toContain("AccessToken");
    expect(enabledMarkup).not.toContain("provider-token-dot");
  });

  it("sorts provider rows by table sortable fields", () => {
    const rows = [
      {
        id: "provider-1",
        name: "B 渠道",
    ...
    expect(
      sortProviderRows(rows, { field: "concurrency", order: "descend" }).map(
        (row: any) => row.name,
      ),
    ).toEqual(["A 渠道", "B 渠道"]);
    expect(
      sortProviderRows(rows, { field: "name", order: "ascend" }).map(
        (row: any) => row.name,
      ),
    ).toEqual(["A 渠道", "B 渠道"]);
  });

  it("adds sort buttons to sortable list columns without touching operation columns", () => {
    const columns = withSortableColumns<any>(
      [
        { title: "名称", dataIndex: "name", width: 120 },
        { title: "数量", key: "count", width: 80 },
        { title: "操作", fixed: "right", width: 80, render: () => "编辑" },
      ],
      { field: "count", order: "descend" },
      ["name", "count"],
    );

    expect(columns[0]).toMatchObject({ sorter: true, sortOrder: null });
    expect(columns[1]).toMatchObject({ sorter: true, sortOrder: "descend" });
    expect(columns[2]).not.toHaveProperty("sorter");
  });

  it("sorts list rows by strings, numbers, booleans and formatted dates", () => {
    const rows = [
      {
        name: "B 来源",
        count: 2,
    ...
    expect(
      sortRowsByTableState(rows, { field: "name", order: "ascend" }).map(
        (row) => row.name,
      ),
    ).toEqual(["A 来源", "B 来源"]);
    expect(
      sortRowsByTableState(rows, { field: "count", order: "descend" }).map(
        (row) => row.name,
      ),
    ).toEqual(["A 来源", "B 来源"]);
    expect(
      sortRowsByTableState(rows, { field: "enabled", order: "ascend" }).map(
        (row) => row.name,
      ),
    ).toEqual(["B 来源", "A 来源"]);
    expect(
      sortRowsByTableState(rows, { field: "updatedAt", order: "ascend" }).map(
        (row) => row.name,
      ),
    ).toEqual(["B 来源", "A 来源"]);
  });
  ...
  it("renders source modal guidance without info alert chrome", () => {
    const payloadHelpMarkup = renderPage(<SourcePayloadSampleHelp />);
    const inboundNoteMarkup = renderPage(<SourceInboundTestNote />);

    expect(payloadHelpMarkup).toContain(
      "展示最近鉴权通过且 JSON 合法的入站 Payload 样例。",
    );
    expect(payloadHelpMarkup).toContain("source-payload-sample-help");
    ...
    expect(inboundNoteMarkup).not.toContain("ant-alert-info");
  });
  ...
  it("renders source dedupe and rate limit policies as quiet inline text", () => {
    const enabledMarkup = renderPage(
      <SourcePolicyCell value="10 分钟" enabled />,
    );
    const disabledMarkup = renderPage(
      <SourcePolicyCell value="未开启" enabled={false} />,
    );

    expect(summarizeSourceDedupe(true, { ttl_seconds: 600 })).toBe("10 分钟");
    expect(summarizeSourceDedupe(true, { ttl_seconds: 86400 })).toBe("1 天");
    expect(enabledMarkup).toContain("source-policy-cell");
    expect(enabledMarkup).toContain("source-policy-cell__dot");
    expect(enabledMarkup).toContain("10 分钟");
    expect(enabledMarkup).not.toContain("ant-tag");
    expect(enabledMarkup).not.toContain("分钟去重");
    expect(disabledMarkup).toContain("未开启");
  });
  ...
  it("renders unified message status as quiet inline text instead of split inbound/outbound state", () => {
    const markup = renderPage(<MessageStatusCell value="queued" />);

    expect(markup).toContain("待发送");
    expect(markup).toContain("message-status-cell");
    expect(markup).not.toContain("premium-status-tag");
  });
  ...
  it("builds source access guide with real source code auth values and generic body", async () => {
    ...
    expect(guide).toContain(
      "入站 URI：https://push.example.com/api/v1/ingest/orders",
    );
  ...
  it("renders empty source IP allowlist as dash", () => {
    ...
    expect(
      renderPage(<SourceAllowlistCell items={["10.20.0.0/16"]} />),
    ).toContain("10.20.0.0/16");
  });
  ...
  it("renders simplified source form controls for token, dedupe, rate limit, and immutable code", () => {
    ...
    expect(enabledMarkup).toContain(
      '<label class="ant-form-item-required" title="鉴权方式">鉴权方式</label>',
    );
    expect(enabledMarkup).toContain("Authorization: Bearer source_token");
    expect(enabledMarkup).not.toContain(
      "只接受 Authorization: Bearer source_token",
    );
    expect(enabledMarkup).not.toContain("不支持 X-MGP-Token");
    expect(enabledMarkup).toContain("留空代表允许 any");
    expect(enabledMarkup).toContain(
      "示例：192.168.66.0/24, 172.16.30.0/24, 127.0.0.1, 172.169.10.11-172.169.10.13",
    );
  ...
  it("builds source input with current defaults and no legacy compatibility reads", () => {
    ...
    expect(input.do_not_disturb_config).toEqual({
      enabled: false,
      windows: [],
    });
    ...
    expect(rateLimitedInput.rate_limit_config).toEqual({
      enabled: true,
      per_second: 15,
    });
    ...
    const mixedAllowlistInput = sourceInputFromDraft({
      ...
      code: "orders",
      ipAllowlistText:
        "192.168.66.0/24,172.16.30.0/24,127.0.0.1\n172.169.10.11-172.169.10.13",
    });
  ...
  it("uses route group source detail payload for route field selectors when source list omits the body", async () => {
    const listSource = {
      id: "source-1",
      code: "orders",
      name: "订单来源",
      latestPayload: "暂无",
      lastInboundAt: "2026-06-04 09:00:00",
      raw: {
        id: "source-1",
        code: "orders",
        latest_payload_sample_updated_at: "2026-06-04T09:00:00Z",
      },
    } as any;
    const detailSource = {
      ...listSource,
    ...
    const group = {
      ...
      ruleIds: [],
      totalHitCount: 0,
      updatedAt: "2026-06-04 09:00:00",
    };

    const options = payloadFieldOptionsFromLatestSamples(
      ConsolePages.routePayloadSourcesForGroup(group, [listSource], {
        "source-1": detailSource,
      }),
    );

    expect(options.map((item) => item.value)).toContain("payload.title");
    expect(options.map((item) => item.value)).toContain("payload.receivers");
    expect(options.map((item) => item.value)).toContain("payload.sender.name");
  });

  it("uses source detail payload for template payload preview when source list omits the body", () => {
    const listSource = {
      id: "source-1",
      code: "orders",
      name: "订单来源",
    ...
    const mergedSources = sourceRowsWithDetails([listSource], {
      "source-1": detailSource,
    });
    const draft = createTemplateDraft(mergedSources, templateCapabilities);

    expect(
      payloadFieldOptionsFromLatestSamples(mergedSources).map(
        (item) => item.value,
      ),
  ...
  it("localizes supported provider labels and exposes them as provider page options", () => {
    ...
    const hiddenLegacyProviderLabels = [
      "短信兼容",
      "企业微信应用兼容",
      "钉钉工作消息兼容",
      "飞书兼容",
    ...
    expect(providersMarkup.indexOf("企业协同")).toBeLessThan(
      providersMarkup.indexOf("个人推送"),
    );
    expect(providersMarkup.indexOf("个人推送")).toBeLessThan(
      providersMarkup.indexOf("邮件短信"),
    );
    expect(providersMarkup.indexOf("邮件短信")).toBeLessThan(
      providersMarkup.indexOf("基础通道"),
    );
    expect(providersMarkup.indexOf("基础通道")).toBeLessThan(
      providersMarkup.lastIndexOf("自建服务"),
    );
  ...
  it("does not expose the removed gov cloud provider in provider configuration", () => {
    ...
    expect(
      providerTypeOptions.some(
        (option) => String(option.value) === "gov_cloud",
      ),
    ).toBe(false);
  ...
  it("uses PushPlus recipient-token provider fields and HTML content template fallback", () => {
    ...
    expect(providerMarkup).toContain("渠道并发上限");
    ...
    expect(providerInput.retry_policy).toEqual({
      max_attempts: 2,
      delay_ms: 3000,
      idempotency_key: "${message_id}:${provider_instance_id}",
    });
    ...
    const templateBody = JSON.parse(templateInput.template_body) as Record<
      string,
      string
    >;

    expect(templateInput.message_type).toBe("html");
    expect(Object.keys(templateBody).sort()).toEqual([
      "content",
      "title",
      "topic",
    ]);
  ...
  it("keeps provider instance more settings when mapping and editing existing channels", () => {
    ...
    const editing = providerWithCapability(
      row,
      providerCapabilityView("serverchan", capabilities),
    );
    const input = channelInputFromProvider(editing);
    const capabilityMarkup = renderPage(
      <ProviderCapabilityTabs provider={editing} />,
    );

    expect(row.rateLimitEnabled).toBe(true);
    expect(row.description).toBe("值班告警主通道");
    ...
    expect(editing.deadLetterRetentionDays).toBe(30);
    expect(capabilityMarkup).toContain("渠道并发上限");
    expect(capabilityMarkup).toContain("4");
    expect(input.description).toBe("值班告警主通道");
    expect(input.concurrency_limit).toBe(4);
    expect(input.dead_letter_policy).toEqual({
      policy: "retry_exhausted_or_upstream_error",
      retention_days: 30,
      replay: false,
    });

    const emptyDescriptionRow = mapChannelRow(
      { ...channel, id: "channel-2", description: "" },
      capabilities,
    );
    expect(emptyDescriptionRow.description).toBe("");
    expect(emptyDescriptionRow.description).not.toBe(
      "来自后端推送渠道实例配置",
    );
  });

  it("shows provider capability request details without internal adapter wording", () => {
    const markup = renderPage(
      <ProviderCapabilityTabs provider={createProviderDraft("wecom_app", 1)} />,
    );

    expect(markup).toContain("POST /cgi-bin/message/send");
    expect(markup).toContain("按 msgtype 生成 text/markdown/textcard 应用消息");
    expect(markup).not.toContain("内置企业微信应用 adapter");
  ...
  it("uses WxPusher standard POST fields and HTML content template fallback", () => {
    ...
    expect(
      providerMarkup.slice(
        providerMarkup.indexOf("Topic ID 列表"),
        providerMarkup.indexOf("描述"),
      ),
    ...
    expect(
      channelInputFromProvider({
        ...providerDraft,
        fieldValues: { "auth_config.app_token": "AT_xxx" },
      }).auth_config,
    ).toEqual({
      app_token: "AT_xxx",
    });
  ...
  it("renders WxPusher test panel with explicit UID and topic fields", () => {
    const markup = renderPage(
      <ProviderTestPanel
        value={createProviderDraft("wxpusher", 1)}
        onChange={() => undefined}
      />,
    );
  ...
  it("builds generic webhook GET test payload with identity recipient and body wrapper", () => {
    const draft = {
      ...
      fieldValues: {
        ...
        "send_config.url":
          "https://21329.push.ft07.com/send/%7B%7B%20identity%20%7D%7D.send",
  ...
  it("renders Feishu robot test panel with OpenID and text payload", () => {
    ...
    expect(markup).toContain("provider-token-status-line");
    expect(markup).not.toContain("ant-alert-info");
    ...
    expect(payload.resolved_recipients).toEqual([
      { platform_ids: { feishu_open_id: "ou_123" } },
    ]);
  });
  ...
  it("uses ServerChan v3 URL-only fields and Markdown template fallback", () => {
    const providers = [["serverchan", "API URL"]] as const;
    ...
    for (const [providerType, credentialLabel] of providers) {
      ...
      const markdownInput = templateVersionInputFromDraft(
        createTemplateDraft([], [], providerType, "markdown"),
      );
      const markdownBody = JSON.parse(markdownInput.template_body) as Record<
        string,
        string
      >;
  ...
  it("uses P2 provider fields and notice template fallbacks", () => {
    ...
    for (const [providerType, credentialLabel, extraLabels] of providers) {
      ...
      const input = templateVersionInputFromDraft(
        createTemplateDraft([], [], providerType, "notice"),
      );
  ...
  it("uses WeCom robot webhook-only channel fields and recipient key message content", () => {
    ...
    expect(
      schema.fields.find((field) => field.key === "msgtype")?.enum,
  ...
  it("uses Bark recipient-only channel fields and message-level option fields", () => {
    ...
    expect(templateInput.message_type).toBe("notice");
    expect(Object.keys(templateBody)).toEqual([
      "title",
      "subtitle",
      "body",
      "group",
    ...
    expect(levelSchema?.enum).toEqual([
      "critical",
      "active",
      "timeSensitive",
      "passive",
    ...
    expect(
      schema.fields.find((field) => field.key === "markdown")?.format_hint,
    ).toBe("支持 Markdown");
    ...
    const markdownBody = JSON.parse(
      templateVersionInputFromDraft(markdownDraft).template_body,
    ...
    expect(Object.keys(markdownBody)).toEqual([
      "title",
      "subtitle",
      "markdown",
      "group",
  ...
  it("uses vendor fields and template/content fallback schemas for the first SMS providers", () => {
    ...
    for (const [providerType, credentialLabel, configLabel] of providers) {
      ...
      const templateInput = templateVersionInputFromDraft(
        createTemplateDraft([], [], providerType, "template"),
      );
      const textInput = templateVersionInputFromDraft(
        createTemplateDraft([], [], providerType, "text"),
      );
  ...
  it("uses robot markdown and enterprise card template fallbacks without exposing raw provider enums", () => {
    const robotTypes = [["wecom_robot", "企业微信群机器人"]] as const;
    const appTypes = [["wecom_app", "企业微信应用消息"]] as const;
    ...
    const dingTalkInput = templateVersionInputFromDraft(
      createTemplateDraft([], [], "dingtalk_robot", "markdown"),
    );
    const dingTalkBody = JSON.parse(dingTalkInput.template_body) as Record<
      string,
      string
    >;
    ...
    for (const [providerType, label] of appTypes) {
      ...
      const input = templateVersionInputFromDraft(
        createTemplateDraft([], [], providerType, "card"),
      );
      expect(markup).toContain(label);
    ...
    const dingTalkWorkMarkdown = createTemplateDraft(
      [],
      [],
      "dingtalk_work",
      "sampleMarkdown",
    ...
    const dingTalkWorkMarkup = renderPage(
      <TemplateEditorForm
        value={dingTalkWorkMarkdown}
        onChange={() => undefined}
        sourceRows={[]}
        capabilities={[]}
      />,
    );
    const dingTalkWorkInput =
      templateVersionInputFromDraft(dingTalkWorkMarkdown);
    const dingTalkWorkBody = JSON.parse(
      dingTalkWorkInput.template_body,
    ) as Record<string, string>;
    ...
    const dingTalkWorkText = templateVersionInputFromDraft(
      createTemplateDraft([], [], "dingtalk_work", "sampleText"),
    );
    const dingTalkWorkTextBody = JSON.parse(
      dingTalkWorkText.template_body,
    ) as Record<string, string>;
  ...
  it("switches DingTalk template fields from the content format selector", () => {
    const robotDraft = createTemplateDraft(
      [],
      [],
      "dingtalk_robot",
      "markdown",
    ...
    expect(
      Object.keys(
        JSON.parse(
          templateVersionInputFromDraft(robotText).template_body,
        ) as Record<string, string>,
      ),
    ).toEqual(["content"]);
    ...
    expect(
      Object.keys(
        JSON.parse(
          templateVersionInputFromDraft(robotMarkdown).template_body,
        ) as Record<string, string>,
      ),
    ).toEqual(["title", "text"]);

    const workDraft = createTemplateDraft(
      [],
      [],
      "dingtalk_work",
      "sampleMarkdown",
    ...
    expect(
      Object.keys(
        JSON.parse(
          templateVersionInputFromDraft(workText).template_body,
        ) as Record<string, string>,
      ),
    ).toEqual(["content"]);
    ...
    expect(
      Object.keys(
        JSON.parse(
          templateVersionInputFromDraft(workMarkdown).template_body,
        ) as Record<string, string>,
      ),
    ).toEqual(["title", "text"]);
  });
  ...
  it("uses Feishu app robot text-only template and app credentials", () => {
    const providerMarkup = renderPage(
      <ProviderConfigForm
        value={createProviderDraft("feishu_robot", 1)}
        onChange={() => undefined}
        capabilities={[]}
      />,
    );
  ...
  it("uses DingTalk group robot base API, optional secret and markdown-only template", () => {
    const providerMarkup = renderPage(
      <ProviderConfigForm
        value={createProviderDraft("dingtalk_robot", 1)}
        onChange={() => undefined}
        capabilities={[]}
      />,
    );
    ...
    const templateDraft = createTemplateDraft(
      [],
      [],
      "dingtalk_robot",
      "markdown",
  ...
  it("uses DingTalk work OAuth field labels aligned with the new token endpoint", () => {
    const providerMarkup = renderPage(
      <ProviderConfigForm
        value={createProviderDraft("dingtalk_work", 1)}
        onChange={() => undefined}
        capabilities={[]}
      />,
    );
  ...
  it("uses Feishu group message webhook fields and text-only template", () => {
    const providerMarkup = renderPage(
      <ProviderConfigForm
        value={createProviderDraft("feishu_group", 1)}
        onChange={() => undefined}
        capabilities={[]}
      />,
    );
    ...
    const capabilities: ProviderCapabilityApiRecord[] = [
      {
        ...
        channel_config_schema: {
          fields: [
            { key: "base_url", label: "API 基础地址", target: "send_config" },
          ],
        },
        ...
        credential_schema: {
          ...
          properties: {
            server_url: {
              type: "string",
              title: "服务地址",
              default: "https://push.i-i.me",
            },
            push_key: {
              type: "string",
              title: "PushMe Push Key",
              format: "password",
            },
          },
        },
  ...
  it("keeps generic webhook configuration compact without token or mapping tabs", () => {
    const builtInMarkup = renderPage(
      <ProviderConfigForm
        value={createProviderDraft("wecom_app", 1)}
        onChange={() => undefined}
        capabilities={[]}
      />,
    );
    ...
    const webhookMarkup = renderPage(
      <ProviderConfigForm
        value={createProviderDraft("webhook", 1, webhookCapabilities)}
        onChange={() => undefined}
    ...
    expect(webhookMarkup).toContain(
      "{{ identity }} 表示当前接收人的平台身份字段值。",
    );
    expect(webhookMarkup).toContain("请求方法");
    expect(webhookMarkup).toContain("POST");
    expect(
      createProviderDraft("webhook", 1, webhookCapabilities).configFields.find(
    ...
    expect(webhookMarkup).not.toContain(
      "按行维护请求 Header，发送时自动拼接到 HTTP 请求头。",
    );
    ...
    const capabilities: ProviderCapabilityApiRecord[] = [
      {
        ...
        credential_schema: {
          ...
          properties: {
            ...
            auth_mode: {
              type: "string",
              enum: ["token", "hmac", "token_and_hmac", "none"],
              default: "token",
            },
          },
        },
    ...
    const hmacMarkup = renderPage(
      <ProviderConfigForm
        value={{
          ...tokenDraft,
          fieldValues: {
            ...tokenDraft.fieldValues,
            "auth_config.auth_mode": "hmac",
          },
        }}
        onChange={() => undefined}
        capabilities={capabilities}
  ...
  it("opens the provider type selector on the basic channel group for generic webhook", () => {
    const markup = renderPage(
      <ProviderTypeCardSelector value="webhook" onChange={() => undefined} />,
    );
    ...
    expect(markup).not.toContain(
      "通用出站 Webhook，自由定义 JSON、支持在请求头附带凭证秘钥。",
    );
  ...
  it("uses email service presets and encrypted SMTP settings", () => {
    ...
    const serviceField = draft.configFields.find(
      (field) => field.key === "service_provider",
    );
    if (!serviceField) {
  ...
  it("maps persistent token cache statuses for provider rows", () => {
    expect(
      tokenCacheStatusMeta({ is_cached: true, token_cache_status: "cached" }),
    ).toEqual({ label: "已缓存", color: "success" });
    expect(
      tokenCacheStatusMeta({ is_cached: false, token_cache_status: "expired" }),
    ).toEqual({ label: "已过期", color: "warning" });
    expect(
      tokenCacheStatusMeta({ is_cached: false, token_cache_status: "missing" }),
    ).toEqual({ label: "未缓存", color: "default" });
  });

  it("renders route page guardrails and hit counts without exposing raw english enums", () => {
    const markup = renderPage(
      <RoutesPage lastUpdated={lastUpdated} onRefresh={() => undefined} />,
    );
    const routeGroupListMarkup = markup.slice(markup.indexOf("路由组列表"));
    const statusIndex = Math.max(
      routeGroupListMarkup.indexOf('aria-label="状态"'),
      routeGroupListMarkup.indexOf('scope="col">状态</th>'),
    );
  ...
  it("renders route group source binding with vertically aligned source code", () => {
    const markup = renderPage(
      <RouteSourceBindingCell sourceName="告警来源" sourceCode="alerts" />,
    );

    expect(markup).toContain("route-source-binding-cell");
  ...
  it("keeps route group enabled state out of create and edit form", () => {
    const RouteGroupForm = (
      ConsolePages as typeof ConsolePages & {
  ...
  it("shows route working copy base version and checkout action", () => {
    const noteMarkup = renderPage(<RouteWorkingCopyNote baseVersionNo={2} />);
    const historyMarkup = renderPage(
      <RouteVersionHistoryContent
        versions={
          [
            {
            ...
            },
            {
              ...
              flow_id: "flow-1",
              version_no: 4,
              draft_base_version_id: "version-2",
    ...
    expect(noteMarkup).toContain("规则列表");
    expect(noteMarkup).toContain("基于 v2 编辑中");
    expect(historyMarkup).toContain("基于 v2 编辑中");
    expect(historyMarkup).toContain("基于此版本编辑");
  });
  ...
  it("renders route send action group rows and supports multiple targets", () => {
    ...
    const draft = {
      ...createRouteRuleDraft(templateRows, channelRows, [
        { label: "严重程度", value: "payload.severity", type: "string" },
      ]),
      targets: [
        {
          id: "target-1",
          channelId: "channel-wecom",
          templateVersionId: "version-wecom",
          enabled: true,
        },
        {
          id: "target-2",
          channelId: "channel-email",
          templateVersionId: "version-email",
          enabled: true,
        },
      ],
    ...
    const markup = renderPage(
      <RouteRuleForm
        ...
        payloadFieldOptions={[
          { label: "标题", value: "payload.title", type: "string" },
        ]}
      />,
    );
    ...
    expect(markup).toContain("route-expression-preview");
    expect(markup).not.toContain("ant-alert-info");
    ...
    expect(
      markup.match(/identity-delete-icon-button/g)?.length,
  ...
  it("renders condition group node editing with an editable name instead of basic node metadata only", () => {
    ...
    const markup = renderPage(
      <RouteConditionGroupEditor
        ...
        matchGroupRows={[]}
        payloadFieldOptions={[
          { label: "消息级别", value: "payload.level", type: "string" },
        ]}
      />,
    );
  ...
  it("validates condition node drafts without requiring send action targets", () => {
    ...
    expect(validateRouteRuleDraft(draft, [], [])).toBe(
      "请至少配置一个发送目标",
    );
  });
      ...
      <ReactFlowProvider>
        <NodeView
          ...
          data={{
            ...
            title: "高优先级条件组",
            description:
              "payload.level = critical 且 payload.scope 包含 华东 且 payload.sender.department = 应急办 且 payload.title 匹配正则 ^P[0-9]+",
            routeDraft: {
              ...
              conditions: [
                ...
                {
                  fieldPath: "payload.scope",
                  operator: "contains",
                  value: "华东",
                  matchGroupIds: [],
                },
                {
                  fieldPath: "payload.sender.department",
                  operator: "equals",
                  value: "应急办",
                  matchGroupIds: [],
                },
  ...
  it("renders route canvas as rule-row actions instead of a free node library", () => {
    const markup = renderPage(
      <RouteCanvasToolbar
        onAddRule={() => undefined}
    ...
    expect(markup).toContain("重置布局");
    expect(markup).toContain("模拟运行");
    expect(markup).toContain("保存画布");
    expect(markup).not.toContain("节点库");
    expect(markup).not.toContain("点击或拖拽新增");
    expect(markup).not.toContain("删除选中");
  });

  it("renders route send action summaries with separated white tooltip rows", () => {
    const markup = renderPage(
      <RouteSendGroupSummaryCell
        value="企业微信实例 -> 企微模板、邮件实例 -> 邮件模板"
        maxWidth={300}
      />,
    );
    const tooltipMarkup = renderPage(
      <RouteSendGroupTooltipCard
        items={["企业微信实例 -> 企微模板", "邮件实例 -> 邮件模板"]}
      />,
    );
    ...
    expect(tooltipMarkup.match(/route-send-group-tooltip-row/g)).toHaveLength(
      2,
    );
  ...
  it("renders route condition summaries with separated white tooltip rows", () => {
    const value =
      "payload.severity = 严重 且 payload.env = prod 或 payload.source = ops";
    const markup = renderPage(
  ...
  it("renders simple canvas nodes without noisy source and fallback descriptions", () => {
    ...
    expect(markup).not.toContain(
      '<span class="route-flow-node__meta">无条件</span>',
    );
  ...
  it("renders canvas node editor footer with delete and save actions only", () => {
    expect(
      ConsolePages.canvasNodeEditorFooter({
        nodeId: "source-start",
        kind: "source",
      } as any),
    ).toBeNull();
    const markup = renderPage(
      <>
        {ConsolePages.canvasNodeEditorFooter(
          { nodeId: "rule-1-condition", kind: "condition" } as any,
          { onDelete: () => undefined, onSave: () => undefined },
        )}
      </>,
    );
  ...
  it("detects unbound business nodes before saving route canvas snapshots", () => {
    const rules = [{ id: "rule-1" }] as any;

    expect(
      ConsolePages.routeCanvasHasUnboundBusinessNodes(
        {
          nodes: [
            { id: "source-start", data: { kind: "source" } },
      ...
      ConsolePages.routeCanvasHasUnboundBusinessNodes(
        {
          nodes: [
            { id: "source-start", data: { kind: "source" } },
            { id: "rule-1-condition", data: { kind: "condition" } },
            { id: "rule-1-recipient", data: { kind: "recipient" } },
            { id: "rule-1-send-group", data: { kind: "send_group" } },
          ],
          edges: [],
        } as any,
        rules,
  ...
  it("renders payload recipient path only for payload recipient strategy", () => {
    ...
    const markup = renderPage(
      <RouteRuleForm
        ...
        channelRows={[]}
        payloadFieldOptions={[
          { label: "接收人", value: "payload.receivers", type: "array" },
        ]}
      />,
    );
  ...
  it("filters route target templates by the selected platform provider type", () => {
    ...
    const wecomMarkup = wecomOptions.map((option) =>
      renderToStaticMarkup(<>{option.label}</>),
    );
    const emailMarkup = emailOptions.map((option) =>
      renderToStaticMarkup(<>{option.label}</>),
    );

    expect(channelOptions.map((option) => option.title)).toEqual([
      "企业微信实例",
      "邮件实例",
    ]);
    expect(wecomOptions.map((option) => option.title)).toEqual([
      "企微模板",
      "未声明平台模板",
    ]);
    expect(emailOptions.map((option) => option.title)).toEqual([
      "邮件模板",
      "未声明平台模板",
    ]);
    ...
    expect(wecomOptions.map((option) => option.value)).toEqual([
      "version-wecom",
      "version-unknown",
    ]);
    expect(emailOptions.map((option) => option.value)).toEqual([
      "version-email",
      "version-unknown",
    ]);
  });
  ...
  it("renders route simulation as a visual trace before raw JSON", () => {
        ...
        result={{
          ...
          rule_results: [
            ...
            },
            {
              ...
              evaluated: false,
              duration_ms: 0,
              stop_reason: "coarse_filter",
              skip_reason: "missing_field:payload.severity",
              slow_rule: true,
            },
          ],
        }}
    ...
    expect(markup).toContain("流程进度");
    expect(markup).toContain("开始");
    expect(markup).toContain("条件已命中");
    expect(markup).toContain("接收人就绪");
    expect(markup).toContain("发送动作组就绪");
    expect(markup).toContain("停止匹配");
    expect(markup).toContain("第一条命中后停止");
    expect(markup).toContain("高优先级");
    expect(markup).toContain("兜底");
    expect(markup).toContain("缺字段");
    expect(markup).toContain("粗过滤跳过");
    expect(markup).toContain("payload.severity 缺失");
    expect(markup).toContain("慢规则");
    expect(markup).toContain("原始 JSON");
  });

  it("renders route simulation scope as a quiet note instead of an alert", () => {
    const markup = renderPage(<RouteSimulationScopeNote />);

    expect(markup).toContain("route-simulation-note");
      ...
      validateRouteRuleDraft(
        {
          ...baseDraft,
          targets: [
            { ...baseDraft.targets[0], templateVersionId: "version-1" },
          ],
        },
        [
          {
        ...
        [
          { id: "channel-1", name: "Webhook 渠道", providerType: "webhook" },
        ] as any,
      ),
  ...
  it("renders template page list mappings with localized provider and validation labels", () => {
    ...
    expect(markup).toContain(
      "创建模板，将下级推送Payload转换为对应渠道支持的字段及文本格式。",
    );
  ...
  it("renders template list cells with concise source, provider icon and dot validation status", () => {
    const sourceMarkup = renderPage(
      <TemplateSourceCell name="告警来源" code="alerts" />,
    );
    expect(sourceMarkup).toContain("告警来源");
    expect(sourceMarkup).toContain('title="告警来源-【alerts】"');
    expect(sourceMarkup).not.toContain("告警来源 / alerts");

    const statusMarkup = renderPage(
      <TemplateValidationStatusCell value="valid" />,
    );
    expect(statusMarkup).toContain("template-validation-status-cell");
    expect(statusMarkup).toContain("template-validation-status-cell--success");
    expect(statusMarkup).toContain("校验通过");
    expect(statusMarkup).not.toContain("ant-tag");

    const providerMarkup = renderPage(<ProviderTypeCell value="wecom_app" />);
    expect(providerMarkup).toContain("provider-type-cell");
    expect(providerMarkup).toContain("企业微信应用消息");
  });

  it("renders a single template preview and validation action", () => {
    const markup = renderPage(
      <TemplateFeedbackToolbar status="valid" onPreview={() => undefined} />,
    );

    expect(markup).toContain("预览并校验");
    expect(markup).toContain("校验通过");
    expect(markup).not.toContain("后端预览");
    expect(markup).not.toContain("校验模板");
  });

  it("renders template version variables as a compact read-only summary", () => {
    const markup = renderPage(
      <TemplateVersionVariablesCell
        variables={["payload.title", "payload.content", "payload.severity"]}
      />,
    );
    const emptyMarkup = renderPage(
      <TemplateVersionVariablesCell variables={[]} />,
    );

    expect(markup).toContain("template-version-variables");
    expect(markup).toContain("payload.title");
    expect(markup).toContain("+2");
    expect(markup).toContain(
      'title="payload.title、payload.content、payload.severity"',
    );
    expect(markup).not.toContain("ant-tag");
    expect(markup).not.toContain("copy");
    expect(emptyMarkup).toContain("未使用变量");
    expect(emptyMarkup).not.toContain("ant-tag");
  });
  ...
  it("renders PushPlus test panel fields with recipient token input", () => {
    ...
    const payload = providerTestPayload(
      {
        ...draft,
        testRecipient: "push-token-1",
        testBody: "<p>PushPlus 测试消息</p>",
      },
      false,
    ) as {
    ...
    expect(payload.recipient).toBe("");
    expect(payload.resolved_recipients).toEqual([
      { platform_ids: { pushplus_token: "push-token-1" } },
    ]);
  });
  ...
  it("renders WeCom robot test panel with recipient key and typed content", () => {
    ...
    const markup = renderPage(
      <ProviderTestPanel
        value={{ ...draft, testTopic: "markdown" }}
        onChange={() => undefined}
      />,
    );
    ...
    expect(payload.resolved_recipients).toEqual([
      { platform_ids: { wecom_robot_key: "robot-key-1" } },
    ]);
  });
  ...
  it("renders DingTalk robot test panel with access token, text, and title fields", () => {
    ...
    expect(markup).toContain("text");
    expect(markup).toContain("title");
    expect(markup.indexOf('title="text"')).toBeLessThan(
      markup.indexOf('title="title"'),
    );
    expect(markup).toContain("模拟请求");
    expect(markup).toContain("真实发送");
    expect(markup).not.toContain("测试接收人");
    ...
    expect(payload.rendered_message.message_type).toBe("markdown");
    expect(payload.resolved_recipients).toEqual([
      { platform_ids: { dingtalk_robot_access_token: "ding-token-1" } },
    ]);
  });
  ...
  it("renders DingTalk work test panel with UserID conversion and msgKey payload", () => {
    ...
    expect(payload.resolved_recipients).toEqual([
      { platform_ids: { dingtalk_userid: "093102391140051902" } },
    ]);
  });
  ...
  it("renders PushMe test panel with recipient key and builds typed payload", () => {
    ...
    expect(payload.resolved_recipients).toEqual([
      { platform_ids: { pushme_push_key: "pushme-key-1" } },
    ]);
  });
  ...
  it("renders ServerChan test panel with recipient sendKey and builds URL identity payload", () => {
    ...
    expect(payload.resolved_recipients).toEqual([
      {
        platform_ids: {
          serverchan_sendkey: "sctp21329tfauqvvbhe2wpeb5lufz4gz",
        },
      },
    ]);
  });
  ...
  it("renders Bark test panel with required recipient and typed payload", () => {
    ...
    const markup = renderPage(
      <ProviderTestPanel
        value={{ ...draft, testLevel: "timeSensitive" }}
        onChange={() => undefined}
      />,
    );
  ...
  it("renders template editor without an enabled switch for new templates", () => {
    const draft = {
      ...createTemplateDraft([], templateCapabilities),
      sourceId: "src-alert",
    };
    const markup = renderPage(
      <TemplateEditorForm
        value={draft}
        onChange={() => undefined}
        sourceRows={[{ id: "src-alert", code: "alerts", name: "告警来源" }]}
    ...
    expect(markup).toContain("告警来源");
    expect(markup).toContain('title="告警来源-【alerts】"');
    expect(markup).not.toContain("告警来源 / alerts");
    expect(markup).toContain("企业微信应用消息");
  ...
  it("renders delete actions for source provider route group and route rule rows", () => {
    ...
    const sourceMarkup = renderPage(
      <SourceRowActions
        record={{ id: "source-1", name: "来源一" } as any}
        onView={noop}
        onEdit={noop}
        onTest={noop}
        onDelete={noop}
    ...
    const providerMarkup = renderPage(
      <ProviderRowActions
        record={{ id: "channel-1", name: "推送一" } as any}
        onView={noop}
        onEdit={noop}
        onTest={noop}
        onDelete={noop}
    ...
    const routeGroupMarkup = renderPage(
      <RouteGroupRowActions
        record={{ id: "flow-1", name: "路由一" } as any}
        onOpen={noop}
        onEdit={noop}
        onDelete={noop}
      />,
    ...
    const routeRuleMarkup = renderPage(
      <RouteRuleRowActions
        record={{ id: "rule-1", name: "规则一" } as any}
        onMoveUp={noop}
        onMoveDown={noop}
        onEdit={noop}
        onDelete={noop}
  ...
  it("renders template version history with restore action and immutable version content", () => {
    ...
    expect(markup).toContain("路由策略绑定的是具体模板版本");
    expect(markup).toContain("不会自动改写现有路由");
    ...
    expect(templateVersionSamplePayload(historyVersion, latestPayload)).toEqual(
      { content: "最新 Payload 内容" },
    );
    expect(templateVersionSamplePayload(historyVersion, null)).toEqual({
      content: "历史内容",
    });
    expect(markup).toContain("基于此版本恢复");
  });

  it("renders template version detail with received preview and collapsed debug payloads", () => {
    const historyVersion = {
      id: "version-2",
      template_id: "tpl-1",
      version_no: 2,
    ...
    const markup = renderPage(
      <TemplateVersionDetail
        ...
        payloadPreview={{ content: "最新 Payload 内容" }}
      />,
    );

    expect(markup).toContain("模板内容");
    expect(markup).toContain("{{ payload.content }}");
    expect(markup).toContain("接收效果");
    expect(markup).toContain("最新 Payload 内容");
    expect(markup).toContain("查看最近 Payload 与出站 JSON");
    expect(markup).toContain("最近 Payload");
    expect(markup).toContain("出站 JSON");
    expect(markup).toContain('<details class="template-version-debug"');
    expect(markup).not.toContain("保存样例 Payload");
    expect(markup).not.toContain("渲染后消息预览");
    expect(markup).not.toContain("历史内容");
  });

  it("builds explicit route updates for template version dependencies", () => {
      ...
      action: {
        targets: [
          ...
          },
          {
            ...
            template_version_id: "version-other",
            enabled: true,
            sort_order: 10,
          },
        ],
    ...
    const oldVersionIds = new Set(["version-old"]);
    ...
    expect(result.changedTargets).toBe(1);
    expect(result.input.action.targets).toEqual([
      {
        channel_id: "channel-2",
        template_version_id: "version-other",
        enabled: true,
  ...
  it("renders template variables as direct clickable copy text without a copy button", () => {
    type VariableTokenComponent = (props: {
      path: string;
      onCopy: (path: string) => void;
    }) => ReactElement;
    const VariableToken = (
      ConsolePages as typeof ConsolePages & {
        TemplateVariableCopyText?: VariableTokenComponent;
      }
    ...
    const markup = renderPage(
      <VariableToken path="payload.title" onCopy={() => undefined} />,
    );
  ...
  it("renders received previews according to text HTML and Markdown formats", () => {
    ...
    htmlDraft.fieldValues.title = {
      expression: "{{ payload.title }}",
      defaultValue: "",
    };
    htmlDraft.fieldValues.content = {
      expression:
        '<strong>{{ payload.content }}</strong><script>alert(1)</script><iframe srcdoc="<script>alert(2)</script>"></iframe><svg><a xlink:href="javascript:alert(3)">x</a></svg><math href="data:text/html;base64,PHNjcmlwdA=="></math>',
      defaultValue: "",
    };
    const markdownDraft = createTemplateDraft(
      sourceRows,
      [],
      "serverchan",
      "markdown",
    );
    markdownDraft.fieldValues.title = {
      expression: "{{ payload.title }}",
      defaultValue: "",
    };
    ...
    textDraft.fieldValues.body = {
      expression: "<b>{{ payload.content }}</b>",
      defaultValue: "",
    };
    ...
    expect(htmlPreview.html).not.toContain("srcdoc");
    expect(htmlPreview.html).not.toContain("xlink:href");
    expect(htmlPreview.html).not.toContain("data:text/html");
    expect(htmlPreview.html).not.toContain("<svg");
    expect(htmlPreview.html).not.toContain("<math");
    expect(markdownPreview.format).toBe("markdown");
    ...
    expect(markdownPreview.html).toContain(
      "curl -X POST http://127.0.0.1:18080/api/v1/ingest/smoke001",
    );
  ...
  it("switches SMTP email received preview between plain text and HTML formats", () => {
    ...
    htmlDraft.fieldValues.subject = {
      expression: "{{ payload.title }}",
      defaultValue: "",
    };
    htmlDraft.fieldValues.body = {
      expression:
        "<strong>{{ payload.content }}</strong><script>alert(1)</script>",
      defaultValue: "",
    };
    htmlDraft.fieldValues.format = { expression: "html", defaultValue: "" };
    const textDraft = createTemplateDraft(sourceRows, [], "email", "text");
    textDraft.fieldValues.subject = {
      expression: "{{ payload.title }}",
      defaultValue: "",
    };
    textDraft.fieldValues.body = {
      expression: "<strong>{{ payload.content }}</strong>",
      defaultValue: "",
    };
    ...
    const htmlSchema = htmlInput.message_body_schema as {
      fields: Array<{ key: string; enum?: string[] }>;
    };
    ...
    expect(
      htmlSchema.fields.find((field) => field.key === "format")?.enum,
    ).toEqual(["text", "html"]);
  });
  ...
  it("uses PushMe type field to choose the received preview format", () => {
    ...
    markdownDraft.fieldValues.content = {
      expression: "**{{ payload.content }}**",
      defaultValue: "",
    };
    markdownDraft.fieldValues.type = {
      expression: "markdown",
      defaultValue: "",
    };
  ...
  it("changes template content fields when provider and message type switch", () => {
    ...
    const markdownDraft = switchTemplateMessageType(
      initialDraft,
      "markdown",
      templateCapabilities,
    );
  ...
  it("applies the global fallback to payload variables inside template expressions", () => {
    ...
    expect(body.content).toBe(
      "{{ payload.content | default('-') }} / {{ payload.alert.ip | default('-') }}",
    );
    expect(templateRenderedPreview(draft)).toContain("CPU 90% / -");
  });

  it("renders and switches custom JSON template content mode", () => {
    const draft = switchTemplateContentMode(
      createTemplateDraft([], templateCapabilities),
      "custom_json",
    );
    const markup = renderPage(
    ...
    expect(markup).not.toContain("自定义 JSON");
  ...
  it("maps template rows with provider type and message type instead of version id as a message field", () => {
    const row = mapTemplateRow(
      {
        ...
        target_provider_type: "email",
        template_body:
          '{"subject":"{{ payload.title }}","body":"{{ payload.content }}"}',
        message_body_schema: {
          fields: [
    ...
    expect(row.source).toBe("来源 A");
    expect(row.sourceCode).toBe("source-a");
    ...
    const markdownRow = mapTemplateRow(
      {
        ...
        template_body:
          '{"title":"{{ payload.title }}","content":"{{ payload.content }}","type":"markdown"}',
        message_body_schema: {},
    ...
    const htmlRow = mapTemplateRow(
      {
        ...
        id: "tpl-pushme-html",
        template_body:
          '{"title":"{{ payload.title }}","content":"<strong>{{ payload.content }}</strong>","type":"html"}',
      },
      [{ id: "src-1", code: "source-a", name: "来源 A" }],
    );
    ...
    const markdownRow = mapTemplateRow(
      {
        ...
        template_body:
          '{"title":"{{ payload.title }}","markdown":"**{{ payload.content }}**"}',
      },
      [{ id: "src-1", code: "source-a", name: "来源 A" }],
    );
  ...
  it("builds polished organization tree nodes with hover add-child actions", () => {
    ...
    expect(renderToStaticMarkup(<>{tree[0]?.title}</>)).toContain(
      "新增下级组织：数据中心",
    );
    expect(renderToStaticMarkup(<>{tree[0]?.title}</>)).toContain(
      "编辑组织：数据中心",
    ...
    const treeMarkup = renderToStaticMarkup(<>{tree[0]?.title}</>);
    expect(treeMarkup).toContain('title="数据中心 - 1000"');
    expect(treeMarkup).not.toContain("org-tree-node__code");
    expect(createChildOrgDraft(org, [org])).toEqual(
      expect.objectContaining({ parentId: "org-root", code: "100001" }),
    );
  });

  it("generates hierarchical organization codes and tree select options", () => {
    const root: OrgUnitApiRecord = {
      id: "org-root",
      parent_id: "",
      code: "1000",
      name: "数据中心",
    ...
    const rootSibling: OrgUnitApiRecord = {
      ...root,
      id: "org-root-2",
      code: "1001",
      name: "业务中心",
    ...
    expect(orgCodeLengthForLevel(1)).toBe(4);
    expect(orgCodeLengthForLevel(3)).toBe(8);
    expect(generateOrgUnitCode("", [])).toBe("1000");
    expect(generateOrgUnitCode("", [root, rootSibling])).toBe("1002");
    expect(generateOrgUnitCode(root.id, [root])).toBe("100001");
    expect(generateOrgUnitCode(root.id, [root, child])).toBe("100002");
    expect(generateOrgUnitCode(child.id, [root, child])).toBe("10000101");

    const treeData = buildOrgTreeSelectData([root, child]);
    expect(treeData).toEqual([
      expect.objectContaining({
        title: "数据中心",
        value: "org-root",
        children: [expect.objectContaining({ title: "运维部" })],
      }),
    ]);
  });
  ...
  it("renders person management controls for users and identities", () => {
    ...
    const userSection = markup.slice(
      markup.indexOf("人员列表"),
      markup.indexOf("接收人组列表"),
    );
    ...
    const statusIndex = Math.max(
      userSection.indexOf('aria-label="状态"'),
      userSection.indexOf('scope="col">状态</th>'),
    );
    const actionIndex = userSection.indexOf('scope="col">操作</th>');
    expect(statusIndex).toBeGreaterThan(-1);
  ...
  it("renders user identity summaries as compact text instead of status pills", () => {
    ...
    expect(markup).toContain(
      'aria-label="钉钉工作消息（UserID）：093102391140051902',
    );
  ...
  it("renders user profile form without a status switch", () => {
    ...
    const markup = renderPage(
      <UserProfileForm
        ...
        orgTreeData={[{ title: "数据中心", value: "org-1", key: "org-1" }]}
        onChange={() => undefined}
      />,
    );

    expect(markup).toContain("姓名");
    expect(markup).toContain("organization-tree-select");
    expect(markup).not.toContain('title="状态"');
    expect(markup).not.toContain('role="switch"');
  });
  ...
  it("builds grouped identity channel options and readable identity value labels", () => {
    const options = identityChannelCascaderOptions([
      {
        label: "企业微信生产应用",
        value: "channel-wecom-app",
        providerType: "wecom_app",
      },
      {
        label: "企业微信群机器人",
        value: "channel-wecom-robot",
        providerType: "wecom_robot",
      },
      {
        label: "PushPlus 工作号",
        value: "channel-pushplus-work",
  ...
  it("keeps identity channel picking deliberate and selected labels compact", () => {
    const channelOptions = [
      {
        label: "PushMe-test（PushMe）",
        value: "channel-pushme-test",
        providerType: "pushme",
      },
    ];
    ...
    expect(
      identityChannelDisplayRender(["PushMe【1】", "PushMe-test（PushMe）"]),
    ...
    const markup = renderPage(
      <IdentityEditor
        ...
        channelOptions={[
          {
            label: "飞书生产应用",
            value: "channel-feishu-work",
            providerType: "feishu_robot",
          },
        ]}
        onChange={() => undefined}
      />,
  ...
  it("renders message log detail attempts as separate send target blocks", () => {
    const AttemptBlocks = (ConsolePages as Record<string, any>)
      .MessageLogAttemptBlocks;
    const attempts = [
      {
        id: "attempt-wecom",
      ...
      },
      {
        ...
        rendered_message: {
          message_type: "html",
          content: { subject: "paid", html: "<p>paid</p>" },
        },
  ...
  it("renders empty message log attempts without alert chrome", () => {
    const AttemptBlocks = (ConsolePages as Record<string, any>)
      .MessageLogAttemptBlocks;
    const markup = renderPage(<AttemptBlocks attempts={[]} />);

    expect(markup).toContain("暂无出站投递尝试");
    expect(markup).toContain("message-log-empty-state");
    expect(markup).not.toContain("ant-alert");
  });

  it("renders match group values without the old bulk maintenance hint", () => {
    ...
    expect(matchMarkup).toContain("引用状态");
    expect(matchMarkup).toContain("全部引用状态");
    expect(matchMarkup).not.toContain("匹配值可在编辑匹配组时按行批量维护。");
    ...
    expect(settingsMarkup).toContain("性能测试参数");
    expect(settingsMarkup).toContain("performance-test-parameter-panel");
    expect(settingsMarkup).toContain("运行性能测试");
    expect(settingsMarkup).toContain("取消性能测试");
    expect(settingsMarkup).not.toContain("测试消息数");
    expect(settingsMarkup).toContain("来源鉴权模式");
    expect(settingsMarkup).toContain("起始并发");
    expect(settingsMarkup).toContain("结束并发");
    expect(settingsMarkup).toContain("Worker 模式");
    expect(settingsMarkup).toContain("当前系统设置");
    expect(settingsMarkup).toContain("跟随测试并发");
    expect(settingsMarkup).not.toContain("并发范围");
    expect(settingsMarkup).not.toContain("最大并发");
    expect(settingsMarkup).not.toContain("并发候选");
    expect(settingsMarkup).not.toContain("例如 1-64，系统会从低到高逐档实测");
    ...
    expect(settingsMarkup).toContain("点击图表柱子切换并发档位");
    expect(settingsMarkup).not.toContain("阶段指标");
    expect(settingsMarkup).toContain("资源诊断");
    expect(settingsMarkup).toContain("DB 压力");
    expect(settingsMarkup).toContain("队列压力");
    expect(settingsMarkup).toContain("PostgreSQL I/O");
    ...
    expect(settingsMarkup).not.toContain("最高");
    expect(settingsMarkup).toContain("连接池等待");
    expect(settingsMarkup).toContain("队列积压");
  ...
  it("uses selected concurrency stage metrics and hides duplicate or low-value rows", () => {
    ...
    expect(performanceStageRowsForSelection(result, selectedRow)).toEqual([
      {
        key: "dispatch",
        label: "当前档位出站链路",
        count: 2,
  ...
  it("auto-selects current concurrency while running and recommendation after completion", () => {
    expect(
      performanceAutoSelectedConcurrency({
        loading: true,
  ...
  it("calculates dead-letter list offset from the server-side page window", () => {
  ...
  it("summarizes performance confirmation ranges before running", () => {
    ...
    expect(performanceConcurrencyConfirmation("16", "1")).toEqual({
      ...
      levelCount: 16,
      estimatedMessageCount: 136,
    });
  });

  it("uses bounded resource pressure thresholds for performance diagnostics", () => {
    const rowsFixtureBase = {
      concurrency: 1000,
      message_count: 1000,
      actual_worker_count: 1000,
      success_rate: 100,
      accepted_qps: 1200,
      dispatch_qps: 1000,
      completion_qps: 800,
      ...
      recommended: false,
      diagnostics: {
        db_pool_acquire_count_delta: 1000,
        db_pool_wait_count_delta: 2,
        db_pool_wait_duration_delta_ms: 20,
        db_pool_acquired_conns_before: 0,
        db_pool_acquired_conns_after: 0,
        db_pool_total_conns_before: 30,
        db_pool_total_conns_after: 30,
        postgres_max_connections: 100,
        postgres_blocks_read: 0,
        postgres_blocks_hit: 0,
        postgres_temp_bytes: 0,
        postgres_blocks_read_delta: 10,
        postgres_blocks_hit_delta: 100,
        postgres_temp_bytes_delta: 0,
        cpu_count: 8,
        go_max_procs: 8,
        queue_backlog_before: 0,
        queue_backlog_after: 20,
        queue_backlog_delta: 20,
        queue_route_plan_before: 0,
        queue_route_plan_after: 20,
        queue_route_plan_delta: 20,
        queue_send_message_before: 0,
        queue_send_message_after: 0,
        queue_send_message_delta: 0,
        queue_oldest_wait_before: 0,
        queue_oldest_wait_after: 2,
        goroutines_before: 20,
        goroutines_after: 25,
        goroutines_delta: 5,
        goroutine_growth_warning: false,
        memory_alloc_bytes_before: 0,
        memory_alloc_bytes_after: 64 * 1024 * 1024,
        memory_alloc_delta_bytes: 64 * 1024 * 1024,
        memory_sys_bytes_before: 0,
        memory_sys_bytes_after: 128 * 1024 * 1024,
        gc_count_delta: 0,
        gc_pause_total_delta_ms: 5,
      },
    };
    const rows = performanceDiagnosticGaugeRows(rowsFixtureBase);
    ...
    const frequentShortWaitRows = performanceDiagnosticGaugeRows({
      ...rowsFixtureBase,
      diagnostics: {
        ...rowsFixtureBase.diagnostics,
        db_pool_wait_count_delta: 1000,
        db_pool_wait_duration_delta_ms: 16720,
      },
    });
    const frequentShortWaitDBRow = frequentShortWaitRows.find(
      (row) => row.label === "DB 压力",
    );
    expect(frequentShortWaitDBRow?.tone).toBe("ok");
    expect(frequentShortWaitDBRow?.percent).toBeLessThan(42);

    const instantBacklogRows = performanceDiagnosticGaugeRows({
      ...rowsFixtureBase,
      diagnostics: {
        ...rowsFixtureBase.diagnostics,
        queue_backlog_delta: 1000,
        queue_route_plan_delta: 1000,
        queue_oldest_wait_after: 0,
      },
    });
    const instantBacklogQueueRow = instantBacklogRows.find(
      (row) => row.label === "队列压力",
    );
    expect(instantBacklogQueueRow?.tone).toBe("ok");
    expect(instantBacklogQueueRow?.percent).toBe(0);
    expect(instantBacklogQueueRow?.value).toBe("最老 0 s");

    const hotRows = performanceDiagnosticGaugeRows({
      ...rowsFixtureBase,
      diagnostics: {
        ...rowsFixtureBase.diagnostics,
        db_pool_wait_count_delta: 200,
        db_pool_wait_duration_delta_ms: 30000,
        queue_backlog_delta: 1000,
        queue_route_plan_delta: 1000,
        queue_oldest_wait_after: 40,
        gc_pause_total_delta_ms: 250,
      },
    });
    expect(hotRows.find((row) => row.label === "DB 压力")?.tone).toBe("danger");
  ...
  it("renders match group reference status and readonly reference list", () => {
    const referencedMarkup = renderPage(
      <MatchGroupReferenceStatusCell referenceCount={2} />,
    );
    const unreferencedMarkup = renderPage(
      <MatchGroupReferenceStatusCell referenceCount={0} />,
    );
    const listMarkup = renderPage(
      <MatchGroupReferenceList
        references={[
          {
            flowId: "flow-1",
    ...
    expect(referencedMarkup).toContain("已引用");
    expect(referencedMarkup).toContain("2 条");
    expect(referencedMarkup).toContain("match-group-reference-status--used");
    expect(unreferencedMarkup).toContain("未引用");
    expect(listMarkup).toContain("引用路由");
    expect(listMarkup).toContain("告警路由组");
    expect(listMarkup).toContain("严重告警");
    expect(listMarkup).toContain("查看路由组");
    expect(listMarkup).not.toContain("ant-tag");
  });

  it("extracts match group route references from nested route conditions", () => {
      ...
      [
        {
          ...
          condition_tree: {
            operator: "and",
            conditions: [
              { operator: "in_match_group", match_group_id: "mg-1" },
              { operator: "not_in_match_group", match_group_id: "mg-2" },
            ],
          },
          enabled: true,
          action: {
  ...
  it("uses typed editors for system settings instead of raw JSON by default", () => {
    expect(
      settingEditorConfigForKey("runtime.delivery_global_concurrency"),
    ).toMatchObject({
      kind: "integer",
      min: 1,
      max: 100000,
    ...
    expect(
      settingInputValueFromEditor("runtime.delivery_global_concurrency", "64"),
    ).toBe(64);
    expect(
      settingEditorConfigForKey("admin.single_account_mode"),
    ).toMatchObject({
      kind: "boolean",
    });
    expect(
      settingInputValueFromEditor("admin.single_account_mode", "false"),
    ).toBe(false);
    expect(
      settingEditorConfigForKey("dead_letter.processing_mode"),
    ).toMatchObject({
      kind: "select",
    });
    expect(
      settingInputValueFromEditor("dead_letter.processing_mode", "manual"),
    ).toBe("manual");
  });
