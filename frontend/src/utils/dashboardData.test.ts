...
describe("dashboard data mapping", () => {
  it("maps overview api payload into stable console sections", () => {
    ...
    expect(buildOverviewViewModel(overview, "1h").metrics[1]?.delta).toBe(
      "最近 1 小时窗口",
    );
    ...
    expect(viewModel.qpsLatencyTrend).toEqual({
      bars: {
        key: "latency",
        label: "平均耗时",
        points: [180],
      ...
      line: { key: "qps", label: "QPS", points: [0.2], color: "#7c3aed" },
    });
    expect(buildOverviewViewModel(overview, "1h").trendLabels).toEqual([
      "18:00",
    ]);
    expect(viewModel.platformRanking[0]?.id).toBe("channel-1");
    expect(viewModel.platformRanking[0]?.providerType).toBe("通用 Webhook");
    expect(viewModel.platformRanking[0]?.providerTypeKey).toBe("webhook");
  ...
  it("builds chinese queue monitoring rows and cleanup status from api data", () => {
    ...
    expect(viewModel.platformHealth.map((item) => item.health)).toEqual([
      "警告",
      "异常",
    ]);
    ...
    expect(buildQueueMonitoringViewModel(queue, "7d").trendLabels).toEqual([
      "05/09",
    ]);
    expect(viewModel.slowRules[0]?.avgDuration).toBe("320 ms");
    expect(viewModel.cleanupRows[0]?.value).toBe("30 天");
    expect(viewModel.cleanupRows[1]?.status).toBe(
      "单批上限 200，去重键 2，审计 3",
    );
  ...
  it("keeps default fallback cards and cleanup copy fully localized", () => {
    ...
    expect(overview.metrics.map((item) => item.label)).not.toContain(
      "成功发送量",
    );
    ...
    expect(queue.trendSeries.map((item) => item.label)).toEqual([
      "路由规划处理量",
      "出站发送处理量",
      "死信数量",
    ]);
      ...
      trend: [
        {
          ...
          failed: undefined as any,
          qps: NaN,
          avg_duration_ms: undefined as any,
        },
      ],
      platform_rankings: [
        {
