...
- 路由策略：路由组、匹配组。
- 日志监控：消息日志、队列监控、操作审计。
- 组织人员：人员管理、接收人组；人员管理页内维护组织树。
- 系统设置：系统参数。

兼容入口与通知：

- 旧的 `logs`、`queue`、`audit` 页面 key 会统一映射到“日志监控”。
- 右上角实时通知只提供“查看日志监控”入口，不再打开独立日志页。
- 右上角实时通知通过 SSE 自动更新，后端推送间隔由系统参数 `console.polling_interval_seconds` 控制，默认 5 秒。
- 页面列表和详情不会因为通知 SSE 自动重拉；需要依赖首次进入、手动刷新、导航切换或新增/编辑/删除后的局部刷新。
...
- 顶部核心指标顺序为“总接收量 -> 总发送量 -> 成功发送量 -> 失败发送量 -> 成功率 -> 平均 OPS”。空数据、异常值或后端临时缺字段时应显示 `0`、`0.00%` 或安全默认值，不出现 `NaN`。
...
首次初始化管理员需要输入两遍初始密码。修改密码成功后，当前管理员的全部旧会话都会失效，管理台会清理本地 token 并回到登录页。

本地开发联调管理员账号为 `admin`，当前测试密码为 `Mvp12345678`。该密码仅用于本地测试环境，不是产品默认密码，生产环境不得复用。

## 系统设置

路径：`系统设置 -> 系统参数 / 性能测试`

- 系统参数按 key 做类型和范围校验，不再接受任意 JSON 文本。
- 性能测试会临时创建测试来源、渠道、模板和路由，运行结束后自动清理。
- 性能测试会写入推荐的 `runtime.delivery_global_concurrency`，连续触发会被短窗口限频。
...
- 来源名称。
- 来源编码，只使用字母和数字；创建后不可修改。
- 鉴权方式，默认建议 Token。
- 来源 Token，调用方通过 `Authorization: Bearer <source_token>` 传入。
- IP 白名单，生产环境建议配置；多个条目可用逗号或换行分隔，支持 CIDR、单 IP 和 IP 段，例如 `192.168.66.0/24, 172.16.30.0/24, 127.0.0.1, 172.169.10.11-172.169.10.13`。
- 消息免打扰，默认关闭；开启后最多配置 5 个时间段，按服务端本地时间判断，支持跨天，例如 `22:00` 至 `08:00`。命中时间段的入站消息会保存日志并显示为“已静默”，不会进入任何推送渠道。

入站样例：

- 最近 payload 样例来自“鉴权通过且 JSON 合法”的入站请求。
- 不要求路由、模板或接收人已经配置成功。
- 在来源列表操作列点击“查看”展示最近 Payload；新增和编辑来源时不展示最近 Payload 页。
...
- AccessToken 刷新使用渠道测试区域中的刷新按钮触发。该操作由后端访问上级 token 接口并同步本地缓存；前端只展示刷新状态和时间，不展示明文 token。
...
人员管理页左侧展示组织树，右侧展示人员列表和筛选结果。组织树节点操作可新增下级组织，系统会自动预置上级组织字段。

手机号和邮箱是基础字段，也是 email/SMS 接收人解析的最终兜底。若同一人员配置了渠道实例级身份或 provider 默认身份，发送规划会优先使用平台身份字段；未配置时才使用基础邮箱或手机号。

人员详情保存会一次性提交基础资料和平台身份集合。保存失败时不会只保存姓名、手机号、邮箱而丢失部分身份；若页面停留时间较长导致记录已被其他操作修改，系统会要求刷新后重新保存。
