...
- `provider_type`：渠道类型，例如 `webhook`、`self`、`pushplus`、`wxpusher`、`serverchan`、`email`、`aliyun_sms`、`tencent_sms`、`baidu_sms`、`wecom_robot`、`wecom_app`、`dingtalk_robot`、`dingtalk_work`、`feishu_robot`、`gov_cloud`。兼容 legacy `wecom`、`dingtalk`、`feishu`、`sms`，高级模式保留 `custom_token`。
...
人员使用系统唯一 ID。不同 provider 需要的身份字段放在 `user_identities` 中：
...
发送时根据目标渠道能力选择对应身份字段。缺失时按配置决定报错、跳过或降级。
...
2. 校验来源启用状态。
3. 执行来源鉴权：先校验 CIDR IP 白名单，再按来源配置校验 `Authorization: Bearer` Token 或 HMAC。
4. 生成 trace_id。
5. 执行入站去重。
6. 写入 `message_records`，并用该 payload 更新来源的 `latest_payload_sample`。
7. 写入 `jobs(type=route_plan)`。
8. 返回 `202 accepted` 或标准错误码。

同步返回只覆盖入站接收阶段：来源不存在或停用、IP 白名单不通过、鉴权失败、JSON 非法、payload 超限、入站限流、入站去重和队列写入失败。`202 accepted` 只表示网关已经接收并排队，不表示已经完成路由、模板渲染或推送渠道发送。
...
- 入站去重：防止下级重复调用。支持完整 payload、字段集合、表达式。
...
- 对选择 `none` 鉴权的来源，管理台必须提示建议配置 CIDR IP 白名单。
...
- 管理台总览：24h 趋势、QPS、成功率、失败排行、平台排行。
