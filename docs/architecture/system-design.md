...
MVP Push Gateway 是一个综合消息推送网关。它面向内部系统、政务系统、业务平台和运维工具，统一接收入站消息，按条件路由、模板、接收人和上级平台能力投递到飞书、钉钉、企业微信、邮箱、短信、随申办政务云、本平台、通用 Webhook 或自定义 Token 平台。
...
- `provider_type`：平台类型，例如 `wecom`、`feishu`、`dingtalk`、`email`、`sms`、`gov_cloud`、`self`、`webhook`、`custom_token`。
- `delivery_channel`：具体可投递实例，例如“上海政务云短信”、“企业微信生产机器人”、“飞书审批通知”。
...
- 是否允许无接收人。
- Token 获取 URL、方法、固定参数、请求头、请求体。
- Token 从响应哪个字段读取。
- Token 有效期从哪个字段读取。
- Token 发送时放在哪里：`query`、`header`、`body`。
- 发送请求 URL、方法、query、headers、body 模板。
- 主动限流配置：QPS、每分钟上限、并发上限、突发容量。
- 超时、重试和死信策略。

每个平台实例必须独立限流、独立并发控制和独立失败隔离。一个慢平台只能积压自己的 `send_message` job，不能阻塞其他平台的发送 worker。

### 组织人员

人员使用系统唯一 ID。不同平台需要的身份字段放在 `user_identities` 中：
...
发送时根据目标平台能力选择对应身份字段。缺失时按配置决定报错、跳过或降级。
...
- 每条入站消息从第一条启用策略开始判断；命中第一条策略后执行该策略的一个或多个动作，并停止继续向下遍历。
- 动作包含模板、目标平台集合、接收人策略、去重策略、失败策略。
- 发布时编译成后端可直接执行的规则模型。
...
- 预解析 JSONPath 或点路径。
- 展开动作节点中的模板版本、平台实例、接收人策略和去重策略。
- 生成粗过滤索引，例如“本规则依赖字段集合”“可能用到的匹配组 ID”“需要的消息类型”。
- 校验无效引用，避免 worker 执行时才发现模板、平台或接收组缺失。
...
- 右上编辑模板和消息体。
- 右下实时预览。
- 保存前做模板语法校验、平台消息体 schema 校验、接收人字段校验。
- 校验失败不允许保存，返回标准错误码。
...
- `Validate(source, sample_payload, provider_schema)`：保存前校验。
...
- 模板只负责生成消息字段、Markdown 文本、Webhook body 或平台请求片段。
- 保存模板时必须完成模板编译、样例 payload 渲染、JSON Schema 校验、平台能力校验和接收人字段兼容性校验。
...
同步返回只覆盖入站接收阶段：来源不存在或停用、IP 白名单不通过、鉴权失败、JSON 非法、payload 超限、入站限流、入站去重和队列写入失败。`202 accepted` 只表示网关已经接收并排队，不表示已经完成路由、模板渲染或上级平台发送。

路由、模板、接收人、上级平台 Token、实际发送和死信属于异步阶段。异步阶段失败不会改变已经返回给下级的 `202 accepted`，只记录在消息日志、出站尝试和队列监控中，通过 `trace_id` 排查。
...
7. 解析接收人。
8. 渲染模板。
9. 生成一个或多个 `delivery_attempts` 和 `send_message` job。
10. 无命中路由时记录标准错误码。
...
3. 按平台实例进入独立并发控制和主动限流。
4. 执行发送前去重。
5. 加载平台配置和能力。
6. 必要时用 secret 换 token。
7. 按平台能力放置 token 和接收人字段。
8. 发起 HTTP/SMTP/短信 SDK 请求，执行平台级超时。
9. 记录请求、响应、耗时、错误码。
...
- 上游平台慢时，只积压该平台实例的 `send_message` 队列，不阻塞其他平台。
...
发送任务在 HTTP 调用后崩溃存在上级平台已收到但本地未记录完成的可能。第一版通过发送前去重、请求快照、trace_id 和可配置重试策略降低重复影响；若目标平台支持幂等键，发送请求构造器应优先带上网关生成的幂等键。
...
入站去重唯一范围必须包含来源，即同一个 `dedupe_key` 只在同一来源内互斥。发送前去重唯一范围必须包含平台实例，即同一个 `dedupe_key` 只在同一上级平台实例内互斥，避免不同来源或不同平台互相误伤。
...
- `route_plan` 待处理数量。
- `send_message` 待处理数量，支持按平台实例拆分。
- 最老 job 等待时间。
- planning 平均耗时和 P95。
- sending 平均耗时和 P95。
- 每个平台实例失败率、限流次数、重试次数、死信数量。
- 入站数量、出站数量、成功率。
