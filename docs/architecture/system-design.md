...
- 画布模式：单个来源起点、条件节点、接收人节点和发送动作组节点组成的 React Flow 布局快照。
- 传统模式：一行一条规则。

底层发布为统一执行模型：

- 第一版语义源是保存后的规则集；画布快照用于展示和布局，缺失或与规则集不一致时由规则集重建，不作为独立执行源。
- 每个来源只允许存在一个启用的路由组；路由组代表该来源的全部路由策略集合。
- 路由组支持多个发布版本，例如 v1、v2。执行时只使用 `current_version_id` 指向的当前执行版本，切换版本通过原子更新当前版本完成；草稿规则列表始终展示最新未发布草稿，历史发布版本只读查看。
...
“发布路由时编译成执行模型”指：用户编辑并保存的规则集是语义源，点击发布时后端把规则集转换为 worker 可快速执行的结构，写入 `route_versions.compiled_rules`。当前画布保存的是 `canvas_snapshot` 布局快照，不直接参与规则编译。编译内容包括：
...
“按 source 加载当前执行版本并缓存”指：planning worker 收到某来源消息时，不每次都从数据库重新组装路由，而是按 `source_id` 读取当前已发布执行版本并放入内存缓存。缓存 key 包含 `source_id` 和 `route_version_id`；发布新版本后更新版本号或广播失效，使 worker 下一次自动加载新版本。

创建或启用路由组前必须检查同一来源是否已经存在启用路由组。若存在，禁止保存，并提示“路由组已存在”。同一来源的策略变更通过路由版本管理，不通过创建多个启用路由组解决。
